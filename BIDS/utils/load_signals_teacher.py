    ...
    for edf_file in sorted(set(edf_list)):
        ...
        if data_type == "ictal":
            ...
            for i in range(len(indices)):
                st = data_df.iloc[indices[i]]["onset"]
                dur = data_df.iloc[indices[i]]["duration"]
                sp = st + dur
                data = eegData[
                    :, int(st * settings["sampfreq"]) : int(sp * settings["sampfreq"])
                ]
                if verbose:
                    print(f"Data shape: {data.shape}, Start: {st}s, Duration: {dur}s")
                yield (data.T)
        elif data_type == "interictal":
            data = eegData
            if verbose:
    ...
    def _process_raw_data_(self, mat_data):
        ...
        for data in mat_data:
            ...
            if self.type == "ictal":
                i = 1
                if self.verbose:
                    print("ictal oversampling ratio =", self.ictal_ovl_len)
