...
def train_val_test_split_continual_t(
    ictal_X, ictal_y, interictal_X, interictal_y, test_ratio, no_test=False
):
    num_sz = len(ictal_y)
    if not no_test:
    ...
    for i in range(num_sz):
        ...
        if X_temp_ictal.ndim == 3:
            X_temp_ictal = np.expand_dims(X_temp_ictal, axis=0)
            y_temp_ictal = np.expand_dims(y_temp_ictal, axis=0)

        X_temp_interictal = interictal_X[
            i * interictal_fold_len : (i + 1) * interictal_fold_len
        ]
        y_temp_interictal = interictal_y[
            i * interictal_fold_len : (i + 1) * interictal_fold_len
        ]
...
def train_test_split_t(ictal_X, ictal_y, interictal_X, interictal_y, test_ratio):
    ...
    print(
        "Total %d seizures, %f percent of each seizure is used for testing."
        % (num_sz, test_ratio)
    )
    ...
    for i in range(num_sz):
        ...
        X_train_interictal, X_test_interictal = (
            X_temp_interictal[: int(X_temp_interictal.shape[0] * test_ratio)],
            X_temp_interictal[int(X_temp_interictal.shape[0] * test_ratio) :],
        )
        y_train_interictal, y_test_interictal = (
            y_temp_interictal[: int(y_temp_interictal.shape[0] * test_ratio)],
            y_temp_interictal[int(y_temp_interictal.shape[0] * test_ratio) :],
        )
        ...
        X_train_ictal, y_train_ictal, X_test_ictal, y_test_ictal = split_arrays_ictal(
            X_temp_ictal, y_temp_ictal, test_ratio
        )
