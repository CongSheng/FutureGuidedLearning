...
from utils.load_signals_student import PrepDataStudentBIDS
...
def distill_student_model(target, epochs, trials, optimizer_type, alpha, temperature):
    ...
    with open("student_settings_BIDS.json") as k:
    ...
    ictal_X, ictal_y = PrepDataStudentBIDS(
        target, data_type="ictal", settings=student_settings, verbose=False
    ).apply()
    interictal_X, interictal_y = PrepDataStudentBIDS(
        target, data_type="interictal", settings=student_settings, verbose=False
    ).apply()
