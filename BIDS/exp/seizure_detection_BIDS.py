...
from utils.load_signals_teacher import PrepDataTeacherBIDS
...
def train_teacher_model(target, epochs, optimizer_type, patience):
    ...
    with open("teacher_settings_BIDS.json") as k:
    ...
    ictal_X, ictal_y = PrepDataTeacherBIDS(
        target, data_type="ictal", settings=teacher_settings, verbose=False
    ).apply()
    interictal_X, interictal_y = PrepDataTeacherBIDS(
        target, data_type="interictal", settings=teacher_settings, verbose=False
    ).apply()
